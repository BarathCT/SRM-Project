...
const CONFERENCE_COMPONENTS = {
    prefixes: [
        'International Conference on', 'IEEE International Conference on',
        'ACM International Conference on', 'Annual Conference on',
        'International Symposium on', 'World Congress on',
        'Asia-Pacific Conference on', 'European Conference on',
        'National Conference on', 'Workshop on'
    ],
    subjects: [
        'Machine Learning', 'Artificial Intelligence', 'Data Science',
        'Computer Vision', 'Natural Language Processing', 'Deep Learning',
        'Cloud Computing', 'Internet of Things', 'Cybersecurity', 'Blockchain',
        'Big Data Analytics', 'Software Engineering', 'Information Systems',
        'Signal Processing', 'Communication Systems', 'Embedded Systems',
        'Robotics and Automation', 'Sustainable Technologies', 'Smart Systems',
        'Biomedical Engineering', 'Renewable Energy', 'Manufacturing Technologies',
        'Materials Science', 'Nanotechnology', 'Environmental Engineering'
    ],
    suffixes: [
        'and Applications', 'and Technologies', 'and Innovations',
        'Systems', 'Research', 'Advances', 'Engineering', ''
    ]
};

// Conference short names
const CONFERENCE_SHORT_NAMES = [
    'ICML', 'NeurIPS', 'CVPR', 'ICCV', 'AAAI', 'IJCAI', 'ACL', 'EMNLP',
    'ICSE', 'FSE', 'ASE', 'ISSTA', 'SIGMOD', 'VLDB', 'ICDE', 'KDD',
    'WWW', 'WSDM', 'CIKM', 'ICDM', 'SDM', 'PAKDD', 'ECML', 'AISTATS',
    'UAI', 'ICRA', 'IROS', 'RSS', 'CoRL', 'ICAPS', 'HPCC', 'SC',
    'IPDPS', 'CLUSTER', 'CCGrid', 'IC2E', 'EDGE', 'SEC', 'ICSOC', 'SCC',
    'ICWS', 'COMPSAC', 'ICSME', 'SANER', 'ESEM', 'EASE', 'ICST', 'ISSTA',
    ''
];

// Organizers/Institutions
const ORGANIZERS = [
    'IEEE', 'ACM', 'Springer', 'Elsevier', 'IET', 'IFIP',
    'MIT', 'Stanford University', 'Carnegie Mellon University',
    'University of California', 'Georgia Tech', 'ETH Zurich',
    'IIT Delhi', 'IIT Bombay', 'IISc Bangalore', 'NIT Trichy',
    'SRM Institute of Science and Technology', 'VIT University',
    'Anna University', 'BITS Pilani', 'Jadavpur University'
];

// Proceedings publishers
const PROCEEDINGS_PUBLISHERS = [
    'IEEE Xplore', 'ACM Digital Library', 'Springer', 'Elsevier',
    'IOP Publishing', 'CEUR-WS', 'MDPI', 'Atlantis Press',
    'EDP Sciences', 'AIP Publishing', 'World Scientific'
];
...
const ALL_RESEARCH_TOPICS = [
    ...
    'Renewable Energy Systems', 'Environmental Sustainability', 'Robotics',
    'Human-Computer Interaction', 'Data Analytics', 'Wireless Networks'
];
...
function generateUniqueConferenceName() {
    ...
    do {
        ...
        const suffix = Math.random() < 0.5 ? pick(CONFERENCE_COMPONENTS.suffixes) : '';
...
async function run() {
    try {
        ...
        console.log(`🎤 Generating ${MIN_PAPERS}-${MAX_PAPERS} conference papers per user`);
        ...
        for (const user of users) {
            ...
            console.log(`📝 Generating ${papersForUser} conference papers for ${userName} (${userFacultyId})`);

            for (let i = 0; i < papersForUser; i++) {
                const year = randomInt(2020, 2025);
                const { conferenceStartDate, conferenceEndDate } = generateConferenceDates(year);
                ...
                const conferenceType = ['India'].includes(location.country) && Math.random() < 0.4
                    ? 'National'
                    : 'International';
                ...
                const doc = {
                    ...
                    year,
                    conferenceName,
                    conferenceShortName: Math.random() < 0.7 ? pick(CONFERENCE_SHORT_NAMES) : '',
                    conferenceType,
                    ...
                    proceedingsTitle: Math.random() < 0.6 ? `Proceedings of ${conferenceName}` : '',
                    proceedingsPublisher: pick(PROCEEDINGS_PUBLISHERS),
                    isbn: generateISBN(),
                    doi: generateDoi(),
                    pageNo: Math.random() < 0.7 ? `${randomInt(1, 500)}-${randomInt(501, 1000)}` : '',
        ...
        for (let i = 0; i < docs.length; i += batchSize) {
            ...
            try {
                const result = await ConferencePaper.insertMany(batch, { ordered: false });
        ...
        if (finalCount > 0) {
            ...
            // By conference type
            const intlCount = created.filter(p => p.conferenceType === 'International').length;
            const natCount = created.filter(p => p.conferenceType === 'National').length;
            console.log(`\n🌍 Conference Type:`);
            ...
            // By indexing
            const scopusCount = created.filter(p => p.indexedIn === 'Scopus').length;
            const wosCount = created.filter(p => p.indexedIn === 'Web of Science').length;
            console.log(`\n📑 Indexing:`);
            ...
            created.slice(0, 3).forEach((paper, index) => {
                ...
                console.log(`   Conference: ${paper.conferenceName} (${paper.conferenceType})`);
                console.log(`   Location: ${paper.conferenceLocation.city}, ${paper.conferenceLocation.country}`);
