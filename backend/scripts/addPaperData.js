...
function buildPublication(type) {
  switch (type) {
    ...
    default:
      return {
        type: "scopus",
        id: String(randomInt(10_000_000_000, 99_999_999_999)),
      };
  }
}
...
async function run() {
  try {
    ...
    console.log(
      `🎲 Applying RANDOM subject areas with MATCHING categories to ALL users`
    );
    ...
    for (const user of users) {
      ...
      console.log(
        `📝 Generating ${papersForUser} papers for ${userName} (${userFacultyId}) - ${user.role} - ${userDepartment}`
      );

      for (let i = 0; i < papersForUser; i++) {
        const year = randomInt(2020, 2025);
        const { journal, publisher } = generateUniqueJournal(
          userFacultyId,
          userName
        );
    ...
    for (let i = 0; i < docs.length; i += batchSize) {
      ...
      try {
        ...
        console.log(
          `🔍 Attempting individual inserts for batch ${
    ...
    if (finalCount > 0) {
      ...
      const sortedSubjectAreas = Array.from(subjectAreaMap.entries()).sort(
        (a, b) => b[1] - a[1]
      );
      ...
      created.slice(0, 5).forEach((paper, index) => {
        ...
        console.log(
          `   Faculty: ${user?.fullName} (${user?.department} Dept.)`
        );
    ...
    console.log(
      "✨ Paper seeding completed with VALID random subject areas and matching categories!"
    );
    console.log(
      "🎯 Result: Faculty from any department can have papers in any subject area"
    );
    console.log(
      "🔐 Validation: All subject areas have matching categories as required by schema"
    );
    console.log(
      "📋 Publication Types: Using only valid types (scopus, sci, webOfScience, pubmed)"
    );
  } catch (err) {
