...
const createUsersInBatches = async (users) => {
  ...
  for (const batch of batches) {
    try {
      ...
      console.log(
        `✅ Inserted batch: ${result.length} users (Total: ${createdCount})`
      );
    } catch (batchError) {
      if (batchError.writeErrors) {
        console.error(
          `❌ Partial batch failure: ${batchError.writeErrors.length} errors`
        );
...
const run = async () => {
  try {
    ...
    const generateFacultyId = () =>
      `FAC-${String(facultyIdx++).padStart(4, "0")}`;
    ...
    for (const college of COLLEGE_CONFIG) {
      ...
      if (college.hasInstitutes) {
        ...
        for (const institute of college.institutes) {
          ...
          users.push(user);
          console.log(
            `👤 Created Campus Admin for ${college.name} - ${institute.name}: ${user.email}`
          );
        }
      } else {
        ...
        users.push(user);
        console.log(
          `👤 Created Campus Admin for ${college.name}: ${user.email}`
        );
      }
    }
    ...
    const totalDepartments = Array.from(collegeDataMap.values()).reduce(
      (sum, college) => sum + college.departmentCount,
      0
    );
    const facultyPerDept = Math.ceil(TARGET_FACULTY_TOTAL / totalDepartments);

    console.log(
      `📊 Creating ~${facultyPerDept} faculty per department (${totalDepartments} departments)`
    );
        ...
        for (const institute of collegeData.institutes) {
          for (const department of institute.departments) {
            for (
              let i = 1;
              i <= facultyPerDept &&
              users.length < 3 + COLLEGE_CONFIG.length + TARGET_FACULTY_TOTAL;
              i++
            ) {
              const user = {
              ...
              console.log(
                `👤 Created Faculty ${
                  users.length - 3 - COLLEGE_CONFIG.length
                }/${TARGET_FACULTY_TOTAL}: ${user.email} (${collegeName} - ${
                  institute.name
                } - ${department})`
              );
            }
    ...
    const sampleFaculty = await User.find({
      role: "faculty",
      $or: [
