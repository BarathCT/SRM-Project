...
function generateUniqueDoi() {
  ...
  return Math.random() < 0.7
    ? `10.${publisher}/${book}.${year}.ch${chapter}`
    : "";
}
...
function generateUniqueBookTitle() {
  ...
  do {
    ...
    const suffix =
      Math.random() < 0.6 ? pick(BOOK_TITLE_COMPONENTS.suffixes) : "";
...
async function run() {
  try {
    ...
    console.log(
      `📚 Generating ${MIN_CHAPTERS}-${MAX_CHAPTERS} book chapters per user`
    );
    ...
    for (const user of users) {
      ...
      console.log(
        `📖 Generating ${chaptersForUser} chapters for ${userName} (${userFacultyId}) - ${user.role}`
      );
      ...
      try {
        ...
        for (const doc of batch) {
          try {
            ...
            console.error(
              `❌ Failed: "${doc.chapterTitle}" - ${singleError.message}`
            );
          }
        }
      }
    ...
    if (finalCount > 0) {
      ...
      const scopusCount = created.filter(
        (c) => c.indexedIn === "Scopus"
      ).length;
      const wosCount = created.filter(
        (c) => c.indexedIn === "Web of Science"
      ).length;
